...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fuseLocationClient = LocationServices.getFusedLocationProviderCLient(p0: this)
    }
    ...
    private fun setUpMap() {
        ...
        fuseLocationClient.lastLocation.addOnSuccessListener(p0: this) {location ->

            if(location != null) {
                lastLocation = location
                val currentLatLong = latLng(location.latitude, location.longitude)
                placeMarker(currentLatLong)
                mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(currentLatLong, p1:13f))
            }
